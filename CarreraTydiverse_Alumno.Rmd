...
```{r}
cran
#Observamos que la información de los datos está en forma de tabla
glimpse(cran)
#De esta forma vemos los datos en forma de código
```
# dplyr
...
r_arch es la columna 5
country es la columna 9
```{r}
#Seleccionamos las columnas desde la 5 a la 9
cran[,(5:9)]
```
...
```{r}
cran2 <- cran %>% select(c("size":"ip_id"))
```
...
```{r}
cran3 %>% mutate(size_gb = size/(2^20*2^20))
```
...
```{r}
count <- by_package %>% summarise(n())
unique <- by_package %>% summarise(n_distinct(ip_id))
countries <- by_package %>% summarise(n_distinct(country))
med_bytes <- by_package %>% summarise(mean(size))

pack_sum <- sum(count[2],unique[2],countries[2],med_bytes[2])
pack_sum
```
...
```{r}
umbral <- by_package %>% count(package) %>% 
  summarise(quantile(n, probs = 0.99))
top_counts <- top_n(umbral,n = 1)
top_counts
```
...
Las variables son la nota, el sexo y las personas que son
```{r}
library(tidyr)
library(tidyverse)
students %>% pivot_longer(cols = c("male","female"),
